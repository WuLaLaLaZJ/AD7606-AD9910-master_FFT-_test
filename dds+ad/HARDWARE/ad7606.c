...
void ad7606_get_fft_data()
{
	...
	for (i=0;i<fftSize;i++)                                                   
	{
		InPutBuffer[2*i] = ((float)((short)g_tAD.usBuf[0])/32768/2);  //FFT计算前处理，偶数位为实部即实际的电压值		
		InPutBuffer[2*i+1] = 0;	//奇数位为虚部，即0
		g_tAD.usWrite = 0;
		HAL_Delay(1);
	}
}
...
注：FFT 输入数组，偶数位实部的实际电压值+奇数位虚部的0    输出数组为半边频谱，故其大小减半 
************************************************************
*/
